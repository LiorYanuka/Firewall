...
describe("System happy flow", () => {
  test("add -> retrieve -> toggle -> retrieve", async () => {
    await request(app)
      ...
      .send({ values: ["10.0.0.1"], mode: "whitelist" })
      .expect(200);

    const r1 = await request(app).get("/api/firewall/rules").expect(200);
    expect(r1.body).toHaveProperty("ip");

    await request(app)
      .patch("/api/firewall/rules")
      .send({ ips: { values: ["10.0.0.1"], active: false } })
      .expect(201);

    const r2 = await request(app).get("/api/firewall/rules").expect(200);
