...
describe("Universal endpoints", () => {
    test("add ip whitelist", async () => {
        const res = await request(app).post("/api/firewall/ip/add").send({
            values: ["127.0.0.1", "::1"],
            mode: "whitelist",
        });
    ...
    test("add url blacklist", async () => {
        const res = await request(app).post("/api/firewall/url/add").send({
    ...
    test("add port whitelist", async () => {
        const res = await request(app).post("/api/firewall/port/add").send({
            values: [80, 443, 65535],
            mode: "whitelist",
        });
        expect(res.status).toBe(200);
