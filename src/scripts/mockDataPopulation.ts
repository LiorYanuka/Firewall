...
function generateIps(count: number): { ip: string; mode: Mode; status: string; active: boolean }[] {
...
function generateUrls(count: number): { url: string; mode: Mode; status: string; active: boolean }[] {
	const results: { url: string; mode: Mode; status: string; active: boolean }[] = [];
...
function generatePorts(count: number): { port: number; mode: Mode; status: string; active: boolean }[] {
	const results: { port: number; mode: Mode; status: string; active: boolean }[] = [];
...
async function main() {
	...
	await db.insert(ipRules).values(ipData).onConflictDoNothing({ target: [ipRules.ip, ipRules.mode] });
	await db.insert(urlRules).values(urlData).onConflictDoNothing({ target: [urlRules.url, urlRules.mode] });
	await db.insert(portRules).values(portData).onConflictDoNothing({ target: [portRules.port, portRules.mode] });
